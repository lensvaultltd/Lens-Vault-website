...
const Header: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
    return (
        <header className="absolute top-0 left-0 right-0 z-10 text-gray-800">
            <div className="container mx-auto flex items-center justify-between p-6">
                <a href="#" onClick={(e) => { e.preventDefault(); onNavigate('main'); }} className="flex items-center gap-3 text-xl font-bold text-gray-900 hover:text-gray-700 transition">
                    <LogoIcon className="w-10 h-8" />
                    <span>Lens Vault</span>
                </a>
                <nav className="hidden md:flex items-center gap-6 text-gray-600">
                    <a href="#" onClick={(e) => { e.preventDefault(); onNavigate('services'); }} className="hover:text-gray-900 transition-colors">Services</a>
                    <a href="#" onClick={(e) => { e.preventDefault(); onNavigate('pricing'); }} className="hover:text-gray-900 transition-colors">Pricing</a>
                    <a href="#" onClick={(e) => { e.preventDefault(); onNavigate('contact'); }} className="hover:text-gray-900 transition-colors">Contact</a>
                </nav>
                <div className="flex items-center gap-4">
                    <button onClick={() => onNavigate('login')} className="font-semibold px-5 py-2 rounded-md text-white bg-gray-900 hover:bg-gray-800 transition-colors">
                        Login
                    </button>
                </div>
            </div>
...
const CallToActionSection: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => (
    <section className="py-20">
        <div className="container mx-auto px-6">
            <AnimatedSection>
                <div className="bg-gray-900 text-white rounded-2xl p-10 md:p-16 text-center shadow-2xl">
...
const PricingPage: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  const plans: Plan[] = [
    { name: "Individual", description: "For Individuals", setupCost: "₦150,000", retainer: "₦65,000", icon: <ShieldCheckIcon className="w-8 h-8" /> },
    { name: "Influencer", description: "For Social Media Professionals", setupCost: "₦200,000", retainer: "₦85,000", icon: <StarIcon className="w-8 h-8" /> },
    { name: "Brand", description: "For Growing Brands", setupCost: "₦300,000", retainer: "₦120,000", icon: <BriefcaseIcon className="w-8 h-8" /> },
    { name: "Family", description: "For Households", setupCost: "₦450,000", retainer: "₦160,000", icon: <UserGroupIcon className="w-8 h-8" /> },
    { name: "Team / SME", description: "For Small to Medium Enterprises", setupCost: "₦800,000", retainer: "₦380,000", icon: <BriefcaseIcon className="w-8 h-8" /> },
    { name: "VIP", description: "For High-Profile Clients", setupCost: "₦300,000", retainer: "₦180,000", icon: <CrownIcon className="w-8 h-8" /> },
  ];
            ...
            {plans.map((plan, index) => (
                <AnimatedSection key={index}>
                    <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/80 p-8 flex flex-col text-center h-full hover:shadow-xl transition-shadow duration-300">
                        ...
                        <div className="mt-8">
                             <button onClick={() => handleSelectPlan(plan)} className="w-full font-semibold px-6 py-3 rounded-md text-white bg-gray-900 hover:bg-gray-800 transition-colors">
                                Select Plan
                            </button>
                        </div>
                    </div>
...
const LoginPage: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
    ...
    const handleLogin = (e: React.FormEvent) => {
        ...
        // Mock login API call
        setTimeout(() => {
            if (email === 'admin@lensvault.com' && password === 'password') {
